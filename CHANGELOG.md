...
- A failed data fetch in the middle of /score-business now stops the run. It always said it would; it did not. The fetch sat mid-block with nothing reading its exit status, so a later command supplied the blocks status and a run that had lost a critical category carried on to the scoring agents. Two commands that run after the fetch are gated the same way, so a failed indicator calculation cannot pass silently either.
- A company with no annual report on file can be analysed again. There has always been a stated exemption letting such a ticker through, naming SPCX as the case, but as written no ticker could ever satisfy it: it demanded that the filing be the ONLY category not marked clean, while two other categories are routinely marked degraded on every healthy run. The exemption now turns on the severities that actually stop a run, and it refuses a truncated or corrupt validation record rather than treating absent evidence as clean.
- News that matters is no longer discarded because of who carried it. The materiality gate required a whitelisted outlet, and barely one article in a hundred from the real feed carries one, so genuine events were scored unimportant and the analysis reused a snapshot from before them. An event carried by two independent outlets now counts, and so does one whose category is plainly material and whose sourcing cannot be settled either way, because the cost of re-running an analysis is smaller than the cost of trading on a stale one. A single re-post of a story nobody else carries is still ignored.
- Plaintiff law-firm investigation notices no longer force a full re-analysis. These are bought wire slots that name a company and mention an investigation, so they matched the regulatory category on every large holding and triggered fresh work each time. That category now means an action by a regulator or a court.

## v1.21.1 — 2026-09-04

- An ETF thesis that reached the final step used to die there and promote nothing. The prompt's output template showed two of its three evidence lists as plain sentences while the validator required bound references, so a model that followed the template wrote a document the producer refused — and it refused with a Python traceback that named neither the list nor the item, because three lists share one renderer. The template is fixed, and a refusal now says exactly which field and which position, because the document is checked before it is rendered. This is what left a 30%-of-portfolio holding with no thesis on the day the previous release had just restored it.
- /monitor no longer defers a ticker because an OLD thesis says its data was broken. A condition written on a day with a missing price bar records that in its own wording, and that sentence cannot expire — the vendor can restore the bar and the sentence still claims the gap, so the caveat was being carried forward by hand, run after run. Deferral now comes from this run's own measured facts, and a condition whose stated gap no longer exists is called out as needing a rewrite instead.
- /portfolio can now check the principle-citation audit BEFORE it writes the decision log: 'portfolio_log audit' prints the cited / not-cited split the write would persist, and writes nothing. Previously that split was only visible after the log was on disk, where the next run reads it — so a note saying three principles went uncited could sit in the same file as a machine record saying one did, at exit 0 with no warning, and the only fix was to redo the step.
- The news classifier's scratch file no longer gets overwritten when /score-business runs straight into /investment-thesis. The two legs classify different date windows over the same articles, and under one filename the second silently replaced the first — leaving the run record pointing at a result that no longer existed.
- Two stderr warnings that reported the same missing session used to spell out the same list of symbols twice, which reads as two separate outages. When the lists are identical the second now points at the first; when they genuinely differ, both still name their own.
- The warning for a news batch whose articles all arrived with empty bodies said nothing could be judged. That overstated it — the source test is unaffected and the rest falls back to the title — and it prompted a sound decision to be redone by hand. It now says what was actually available and what that can miss, in both directions.
- Smaller: an ETF field the model was invited to write but nothing ever displayed has been removed rather than left to evaporate silently.

## v1.21.0 — 2026-09-03

- An ETF or stock whose price window is missing ONE middle session can be analysed again. A single missing bar used to cut the usable history back to the days AFTER it, and the one-year lookback that gates an ETF thesis then needed a year to recover — one vendor gap kept a 30%-of-portfolio holding at 'analysis unavailable' for four days. History either side of a gap is now used for the questions that do not need consecutive days (is this a new closing high, how deep is the drawdown), while the moving averages, which do, still refuse.
- Two numbers that were quietly wrong whenever a session was missing. The 3-month relative strength measured the stock over 64 sessions against its benchmark's 63, because a dropped bar slid the starting point back a day instead of shortening the window — the error is one day's return, with nothing bounding it. And the run-day indicator block (RSI, MACD, Bollinger, ATR, volume) still reported PASSED while every rolling window that reached across the gap covered an extra session. Eight of twenty symbols were affected on one real day.
- Exact-sounding numbers are no longer published from a window that cannot support them. A holding showed '0.86% above its prior high' — computed over three bars. The prior-high figures now appear only when the full year is actually on hand, and a breakout can no longer be claimed across a gap, though 'below the prior high' still can, because that one is provable either way.
- The portfolio dashboard stopped silently dropping holdings from its equity total. A fund, and any name whose newest run has analysis but no thesis, were both left out — understating equity by 38.5% and inflating every weight, on a page used to judge position size. Both are now priced from what is already on disk, each carrying the date its own price came from.
- New warnings where the run used to be silent: the market-data step now says on stderr which session a provider did not serve and which symbols lost indicator accuracy to it; /portfolio names any of today's analysis runs that are still half-written, so a decision built on yesterday's evidence for that name is visible rather than assumed.
- Smaller: a duplicated advisory line in the decision log; the STALE line now says when an older run's seal cannot name which key moved; an ETF refusal no longer records an analysis agent that never ran; and a news batch whose articles all arrived with empty bodies now says so, instead of reading as a quiet week.

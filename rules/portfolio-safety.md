...
**The executable source is `scripts/compile_strategy.py:_DEFAULT_PRINCIPLES`.**
The list below documents it. Keeping the text only here forced the compiler to
emit an empty `soft_principles` and hand the artifact to orchestration to
repair, which left the shipped example misreporting its own policy. If the two
ever disagree, the code is authoritative and this section is the fix.
...
Hard constraints come from `strategy.yaml`'s **`risk:` block**, projected
deterministically by `scripts/compile_strategy.py`. They are **not** extracted
from the prose of `principles` — that extraction was the F4 fail-open: a value
written under `risk:` was never read, `source_hash` covered only `principles`,
so an edit produced a cache hit and the decision log kept attesting the
superseded policy. The system recognizes these constraint types:
...
| ~~`max_sector`~~ | — | — | **REFUSED at compile time.** Sector lookup is not implemented, so `validate.py` fails closed on it and EVERY run would refuse. `compile_strategy` exits 1 telling the operator to delete the line. It shipped in an older `strategy.example.yaml` and never enforced anything, so an install carrying it must remove it once. Nothing downstream can arm it |
...
All are optional; a key absent from `risk:` is simply not enforced. These are
the canonical keys `scripts/validate.py` recognizes, and the ONLY keys `risk:`
may contain — an unknown key there is **refused**, never dropped, because
dropping it is precisely how an intended constraint silently fails to bind.
...
- **max_sector stub** (HIGH-9): `validate.py` still emits `invalid_config`
  for a `max_sector` that reaches it — but since the compile step now

...
"$PYBIN" -m scripts.clear_stale "$REPORT_DIR/industry_analysis.json" || exit 1
```
...
handler above was ASKED to stamp `run_meta.industry --no-completed` — that
command is guarded so it cannot mask the validation error, which also means
it may itself have failed: check its own stderr before retrying. The agent's
...
First clear the stale destination, in its own block (this shell inherits
nothing from the last one — substitute the concrete `$REPORT_DIR` you
resolved earlier):
...
"$PYBIN" -m scripts.clear_stale "$DELTA_FILE" "$REPORT_DIR/.tier_context.json" || true   # BEST-EFFORT: pure cleanup, not a guard — $DELTA_FILE is `>`-rewritten next run and .tier_context.json is regenerated by full/partial (and unread on no_op), so a leftover cannot be mistaken for this run's output. Both may legitimately be absent already.
```
